...
public class SearchPostsPageController implements Initializable {
    ...
    public void postInitialization (String hashtagWord) {
        ...
        Bridge bridge1 = new Bridge(Commands.SEARCH_MY_POSTS, message);
        SendMessage.send(bridge1, writer);
        try {
            Bridge b = (Bridge) reader.readObject();
            if (b.getCommand() == Commands.SEARCH_MY_POSTS) {
        ...
        Bridge bridge2 = new Bridge(Commands.SEARCH_OTHERS_POSTS, message);
        SendMessage.send(bridge2, writer);
        try {
            Bridge b = (Bridge) reader.readObject();
            if (b.getCommand() == Commands.SEARCH_OTHERS_POSTS) {
                ArrayList<PostObject> posts = b.get();
                ObservableList<PostObject> postsObservable = FXCollections.observableArrayList();
                postsObservable.addAll(posts);
    ...
    public void searchWord () {
        ...
        LinkedInApplication.showSearchPostPage(searchPhrase);
    }
